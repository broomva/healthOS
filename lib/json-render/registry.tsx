...
// AI SDK pins zod v3, so healthCatalog is typed as `any` (see catalog.ts).
// We define explicit prop interfaces here and cast the components map to
// bypass the Catalog constraint — runtime behaviour is unaffected.
...
// ---------------------------------------------------------------------------
// Prop types — mirrors the zod schemas in catalog.ts
// ---------------------------------------------------------------------------

type StatusKey = "good" | "warning" | "critical" | "neutral";
type TrendKey = "up" | "down" | "stable";
type BarColorKey = "blue" | "green" | "yellow" | "red" | "purple" | "teal";
type VariantKey = "default" | "health" | "sleep" | "training" | "live";
type SeverityKey = "info" | "warning" | "critical";
type TextVariantKey = "body" | "caption" | "heading" | "code";
...
type MetricGridProps = { columns?: number };
type DataTableProps = {
	columns: TableColumn[];
	rows: Record<string, unknown>[];
	title?: string;
};
type StatusBadgeProps = { label: string; status: StatusKey };
type ProgressBarProps = { label: string; value: number; color?: BarColorKey };
type SectionCardProps = {
	title: string;
	subtitle?: string;
	variant?: VariantKey;
};
type AlertBannerProps = { message: string; severity: SeverityKey };
type TextBlockProps = { content: string; variant?: TextVariantKey };
type JsonViewerProps = { data: string; title?: string; maxHeight?: number };
type SkillCardProps = {
	name: string;
	description: string;
	installed?: boolean;
...
/** Minimal context shape that defineRegistry passes to each component render fn. */
type Ctx<P> = { props: P; children?: ReactNode };
...
const TREND_ICONS: Record<TrendKey, string> = {
	up: "\u2191",
	down: "\u2193",
	stable: "\u2192",
} as const;

const BAR_COLORS: Record<BarColorKey, string> = {
	blue: "bg-blue-500",
...
const VARIANT_STYLES: Record<VariantKey, string> = {
...
// ---------------------------------------------------------------------------
// Component render functions (explicitly typed to avoid props: unknown)
// ---------------------------------------------------------------------------

const components = {
	MetricCard: ({ props, children }: Ctx<MetricCardProps>) => (
	...
	MetricGrid: ({ props, children }: Ctx<MetricGridProps>) => (
		<div
			className={cn(
				"grid gap-2",
	...
	DataTable: ({ props }: Ctx<DataTableProps>) => (
	...
	StatusBadge: ({ props }: Ctx<StatusBadgeProps>) => (
		<span
			className={cn(
	...
	ProgressBar: ({ props }: Ctx<ProgressBarProps>) => (
	...
	SectionCard: ({ props, children }: Ctx<SectionCardProps>) => (
		<div
			className={cn(
	...
	AlertBanner: ({ props }: Ctx<AlertBannerProps>) => (
		<div
			className={cn(
	...
	TextBlock: ({ props }: Ctx<TextBlockProps>) => {
		const variantClass: Record<TextVariantKey, string> = {
	...
	JsonViewer: ({ props }: Ctx<JsonViewerProps>) => (
	...
	SkillCard: ({ props }: Ctx<SkillCardProps>) => (
...
// Cast components to `any` to satisfy the Catalog constraint that requires
// zod v4 types — runtime behaviour is identical.
