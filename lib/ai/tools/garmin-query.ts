...
const COMMAND_DESCRIPTIONS: Record<string, string> = {
  "context": "Aggregated health snapshot: profile + stats + health + training + activities. Best for general overview.",
  "health sleep": "Sleep data: duration, phases (deep/REM/light), sleep score, SpO2, respiration, HRV overnight.",
  "health heart-rate": "Heart rate data: resting HR, min/max, time series, HR zones.",
  "health steps": "Step count: total steps, distance, time series (15-min intervals).",
  "health stress": "Stress levels: avg/max, time distribution (rest/low/med/high), time series.",
  "health body-battery": "Body battery: charged/drained, highest/lowest, time series, feedback events.",
  "health rhr": "Resting heart rate: current and historical values.",
  "training status": "Training status: productive/maintaining/overreaching, ACWR, acute/chronic load.",
  "training readiness": "Training readiness: score 0-100, factor breakdown (sleep, recovery, HRV, stress, ACWR).",
  "training hrv": "HRV: weekly avg, last night, baseline range, status (balanced/unbalanced/low).",
  "training vo2max": "VO2 Max estimates for running and cycling.",
  "training lactate": "Lactate threshold: speed, heart rate, FTP, power-to-weight.",
  "training endurance": "Endurance score.",
  "training hill": "Hill score.",
  "athlete": "Athlete profile: name, gender, weight, height, birth date, device settings.",
  "athlete stats": "Today's daily stats: steps, distance, calories, HR, floors, active time.",
  "activities list": "Recent activities list: type, duration, distance, HR, calories, elevation.",
  "weight get": "Current weight and body composition: weight, BMI, body fat, muscle mass.",
};
  ...
  execute: async ({ command, args }) => {
    try {
      ...
      if (stderr && stderr.includes("auth")) {
        return {
          error: "Authentication error. The Garmin CLI needs to be re-authenticated. Run: garmin-connect auth login",
          exitCode: 2,
        };
      }
      ...
      if (err.code === "ENOENT") {
        return {
          error: "garmin-connect CLI not found. Install it: curl -fsSL https://raw.githubusercontent.com/eddmann/garmin-connect-cli/main/install.sh | sh",
        };
      }
